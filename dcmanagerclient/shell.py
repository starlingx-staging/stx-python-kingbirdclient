...
class DCManagerShell(app.App):
    ...
    def build_option_parser(self, description, version,
        ...
        parser.add_argument(
            ...
            action='store',
            dest='dcmanager_url',
            default=c.env('OS_DCMANAGER_URL'),
            help='DC Manager API host (Env: OS_DCMANAGER_URL)'
        )

        parser.add_argument(
            '--os-dcmanager-version',
            action='store',
            dest='dcmanager_version',
            default=c.env('OS_DCMANAGER_VERSION', default='v1.0'),
            help='DC Manager API version (default = v1.0) (Env: '
                 'OS_DCMANAGER_VERSION)'
        )

        parser.add_argument(
            '--os-dcmanager-service-type',
            action='store',
            dest='service_type',
            default=c.env('OS_DCMANAGER_SERVICE_TYPE',
                          default='dcmanager'),
            help='DC Manager service-type (should be the same name as in '
                 'keystone-endpoint) (default = dcmanager) (Env: '
                 'OS_DCMANAGER_SERVICE_TYPE)'
        )

        parser.add_argument(
            '--os-dcmanager-endpoint-type',
            action='store',
            dest='endpoint_type',
            default=c.env('OS_DCMANAGER_ENDPOINT_TYPE', default='internalURL'),
            help='DC Manager endpoint-type (should be the same name as in '
                 'keystone-endpoint) (default = internalURL) (Env: '
                 'OS_DCMANAGER_ENDPOINT_TYPE)'
        )
